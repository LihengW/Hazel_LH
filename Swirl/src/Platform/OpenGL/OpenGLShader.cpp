...
namespace Swirl {
	...
	std::string OpenGLShader::ReadFile(const std::string& filepath)
	{
		...
		if (in)
		{
			...
			result.resize(in.tellg()); // get length of the shader info
	...
	std::unordered_map<GLenum, std::string> OpenGLShader::PreProcess(const std::string& source)
	{
		...
		{
			size_t eol = source.find_first_of("\r\n", pos); //End of shader type declaration line (first line in fact)
			bool invalid = (eol != std::string::npos);
			SW_CORE_ASSERT(invalid, "Syntax error");
	...
	void OpenGLShader::Compile(const std::unordered_map<GLenum, std::string>& shaderSources)
	{
		...
		bool isTwoShader = shaderSources.size() <= 2;
		SW_CORE_ASSERT(isTwoShader, "We only support 2 shaders for now");
